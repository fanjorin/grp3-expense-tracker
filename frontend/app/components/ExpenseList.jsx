      ...
      <div className="overflow-x-auto">
        {expenses.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 text-slate-400 gap-3">
            ...
            <p className="text-sm font-medium">No transactions yet. Start by adding one!</p>
          </div>
        ) : (
                ...
                <tr key={expense.id} className="group hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm ${expense.type === 'INCOME' ? 'bg-green-50 text-green-600' : 'bg-blue-50 text-blue-600'}`}>
                  ...
                  {/* Description */}
                  <td className="px-6 py-4">
                    <span className="text-slate-500 font-medium">{expense.description || "—"}</span>
                  </td>

                  {/* Amount */}
                  <td className="px-6 py-4">
                    <div className="flex flex-col">
                      <span className={`font-bold text-base ${expense.type === 'INCOME' ? 'text-green-600' : 'text-slate-900'}`}>
                        {expense.type === 'INCOME' ? '+' : '-'}₦{expense.amount.toLocaleString()}
                      </span>
                      <span className={`text-[10px] font-bold uppercase tracking-tighter ${expense.type === 'INCOME' ? 'text-green-400' : 'text-slate-300'}`}>
                        {expense.type}
                      </span>
                    </div>
                  </td>
