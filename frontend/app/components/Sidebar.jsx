...
export default function Sidebar({ isOpen, onClose }) {
  ...
  return (
    <>
      ...
      <aside
        className={`w-64 min-h-screen bg-white border-r border-slate-100 flex flex-col px-5 py-8 fixed left-0 top-0 z-50 transition-transform duration-300 lg:translate-x-0 ${
      ...
      >
            ...
            <nav className="flex flex-col gap-1.5">
              {navLinks.map((link) => {
                ...
                return (
                  ...
                  >
                    <span
                      className={`text-base transition-transform duration-300 group-hover:scale-110 ${isActive ? "scale-110" : ""}`}
                    >
                      {link.icon}
                    </span>
        ...
        <div className="relative mt-auto overflow-hidden rounded-3xl bg-[#0052CC] p-6 text-white shadow-xl shadow-blue-600/20 group cursor-default">
          ...
          <div className="relative z-10">
            <div className="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center text-xl mb-4 backdrop-blur-sm">
              💎
            </div>
